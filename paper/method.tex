...
There are many ways to measure the flux of a galaxy; each has flaws.
\begin{itemize}
\item Aperture photometry is the measurement of the light in the image
  above the sky level within a fixed circular angular aperture.  This
  kind of photometry obtains different fractions of the galaxy light
  in galaxies that have different proper sizes or which lie at
  different radial (angular-diameter) distances.
\item Aperture photometry can in principle be improved by scaling the
  ...
  angular size) point-spread function illuminate even the properly
  scaled apertures differently.  This change also doesn't account for
  galaxies with different proper sizes.
\item isophotal photometry...
\item azimuthally averaged profiles...
\item Petrosian photometry...
\item fitting flexible models...
\item fitting inflexible models...
\end{itemize}
In some sense, it is not possible to perform accurate photometry on
galaxies.  All these problems with photometric measurements flow into
other kinds of measurements, like sizes, concentrations, surface
brightnesses, and so-on, because these measurements are (or involve)
transformations of photometric measurements.

The best choice among measurement procedures depends on the uses to
which the measurements are going to be put; in some cases precision is
paramount, in others consistency across distance, in others robustness
to varying point-spread function, in others consistency across
morphological types.

Our goals are...so we do...
